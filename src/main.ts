...
const groundMirror = new Reflector(groundGeometry, {
    textureWidth: window.innerWidth * renderer.getPixelRatio(),
    textureHeight: window.innerHeight * renderer.getPixelRatio(),
    clipBias: 0,
    color: 0xb0e0e6,
});
...
for (let adv of adURLs) {
    ...
    const ad = new Advert(
        ...
        adTexture,
        adPanel.scene,
        new THREE.Vector3(1.375, 1.375, 0.01),
        `${filesRoot}${adBasePath}${adv.path}`
    );
    ads.push(ad);
    scene.add(ad._model);
    ad._model.rotateY(-Math.PI / 2);
    ad._model.position.set(15, 0, adZ);
    adZ += 5;
}
...
eventPlatformData.forEach((epd) => {
    const eventPlatform = new Artwork(
        epd.name,
        epd.name,
        epd.texture,
        artworkPanel,
...
const standupPlatform = new Artwork(
    "Arati Kadav",
    "Aarohi 2022",
    pst,
    artworkPanel,
...
const showcasePlatform = new THREE.Mesh(new THREE.BoxGeometry(8, 0.3, 4), showcaseStageMaterial);
...
videoController.schedule(new Date(2022, 3, 2, 15, 45), `${recordingsRoot}Final+Touch.mp4`),
    videoController.schedule(
        new Date(2022, 3, 3, 8, 0),
        `${recordingsRoot}Aarohi22++VNIT+Nagpur++Official+Theme+Release+Video_1080p.mp4`
    );
...
function treasureSpawn() {
    ...
    const coords = new THREE.Vector3((R0 + offset) * Math.cos(theta), 0, (R0 + offset) * Math.sin(theta));
    if (GameState.PlayerState == "FREEROAM") {
        ...
        hud.modal.content.innerText =
            "A Treasure has spawned on the map, be the first to reveal it and send to @aarohi_vnitnagpur for a chance to win exciting prizes";
...
const standeeGometry = new THREE.PlaneGeometry(2, 4, 1, 1);
const winnerStandeeBasePath = `${filesRoot}res/winners/`;
let winners = ["swar.jpg", "rd.png", "dramatics.jpg", "cyno.jpg"];
...
const streamLoungeRing = new THREE.Mesh(
    new THREE.TorusGeometry(5, 0.05, 16, 64),
    new THREE.MeshBasicMaterial({
        color: "gold",
    })
);
...
const crossWordArea = new Artwork(
    "Crossword",
    "Aarohi Crossword",
    crosswordTexture,
    artworkPanel,
...
const hintParaOne =
    "Welcome to the Aarohi Treasure Hunt! Follow the clues across Aarohi's social media platforms. Each clue will lead to a new one. Search through posters and captions for the clues, and answer the questions in our attached Google form as you uncover each clue. Here's your first clue:";
const hintParaTwo =
    'The first mention of "A Universal Symphony" (Hint: Find the boards that say Instagram and Facebook)';
