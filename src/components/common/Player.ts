...
export class Player {
    ...
    motionUpdate(deltaT: number) {
        ...
        rotataeXQuaternion.setFromAxisAngle(new THREE.Vector3(0, 1, 0), this.motion.mouseNormalX * this.lookSpeedX);
        rotataeYQuaternion.setFromAxisAngle(new THREE.Vector3(1, 0, 0), this.motion.mouseNormalY * 0.3);
    ...
    animate() {
        ...
        switch (this.animationState) {
            case "accelerating": {
                ...
                movementTarget.setFromAxisAngle(new THREE.Vector3(1, 0, 0), -Math.PI / 6);
                playerMesh.quaternion.slerp(movementTarget, 0.1);
                break;
            }
            case "decelerating": {
                const movementTraget = new THREE.Quaternion();
                movementTraget.setFromAxisAngle(new THREE.Vector3(1, 0, 0), Math.PI / 6);
                playerMesh.quaternion.slerp(movementTraget, 0.1);
                break;
            }
            case "idle": {
                const movementTraget = new THREE.Quaternion();
                movementTraget.setFromAxisAngle(new THREE.Vector3(1, 0, 0), 0);
                playerMesh.quaternion.slerp(movementTraget, 0.1);
                break;
            }
        }
